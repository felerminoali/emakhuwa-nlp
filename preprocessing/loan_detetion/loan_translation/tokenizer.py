...
class Tokenizer(object):
    ...
    def tokenize_custom(self, line, insert_start=None, insert_end=None, sample=None):
        """tokenize a line, insert_start and insert_end are lists of tokens"""
        inputs = self.segment(line)
        targets = []
        if insert_start is not None:
        ...
        if insert_end is not None:
            targets += insert_end
        return torch.LongTensor(targets), inputs
...
class BPETokenizer(Tokenizer):
    ...
    def set_bpe(self, codes_file=None):
        
        if codes_file:
            codes_file = codes_file
        else:
            codes_file = self.codes_file
        with codecs.open(codes_file, encoding='UTF-8') as codes:
